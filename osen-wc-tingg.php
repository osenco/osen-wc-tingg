...
function init_tingg_gateway_class()
{
	class WC_Gateway_Tingg extends WC_Payment_Gateway
	{
		...
		public function __construct()
		{
			...
			$this->id                 = 'tingg';
			$this->method_title       = 'Tingg';
			$this->method_description = 'Tingg allows you to make and collect payment in 33+ countries in Africa from a single integration';
			...
			$this->iv_key       = $this->get_option('iv_key');
		...
		public function process_payment($order_id)
		{
			...
			$order_country = array_filter($this->COUNTRIES, function ($item) use ($order) {
			...
			$checkout_payment_url = sprintf(
				...
				$this->encryptCheckoutRequest($payload),
				$this->access_key,
		 ...
		 * @return string
		 */
		public function encryptCheckoutRequest($payload)
		{
			$secret_key = hash('sha256', $this->secret_key);
			$iv_key     = substr(hash('sha256', $this->iv_key), 0, 16);
